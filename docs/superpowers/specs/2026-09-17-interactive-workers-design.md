...
| Ambiente do filho | `execFile('tmux', …, { env: workerEnv(process.env, workerId, port) })` | O servidor `-L hive` nasce com o ambiente do primeiro cliente: sem `CLAUDECODE`, com as variáveis do Hive. O comando ainda leva `HIVE_WORKER_ID`/`HIVE_PORT` inline (é o que `workerCommand` faz) |
