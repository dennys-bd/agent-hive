...
| Sem slot livre | `confirm()` nativo: `Nenhum slot livre. Subir máx. workers de N pra N+1 e iniciar #id?`. Sim → a mesma rota com `raiseMax: true` | É o que `kill` já usa; sem modal novo. Uma única requisição evita a corrida entre subir o teto e iniciar |
...
| `lastEvent` | `'iniciado à mão'` no lugar de `'iniciando'` | O card e o `hive.log` (via `describeChanges`) mostram que não foi a porta automática |
