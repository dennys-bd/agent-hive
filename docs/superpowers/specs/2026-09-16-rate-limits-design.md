...
| Validação (`parseRateLimits`) | Chave `/^[a-z][a-z0-9_]{0,31}$/`, no máximo 8 janelas; `used_percentage` finito ≥ 0, limitado a 100; `resets_at` inteiro > 0 (segundos) → ISO. `isRateLimits` aplica os mesmos limites ao `state.json`. Janela sem os dois campos é descartada; nenhuma válida → nada é despachado | Qualquer processo local pode bater na rota; o estado nunca recebe forma ou tamanho arbitrário |
