...
export async function bootHive(repo: string, deps: Pick<ServerDeps, 'readPlanLimits'> = {}): Promise<BootedHive> {
  ...
  if (!config) return bootSetupMode(repo, log, deps);
  ...
  try {
    ...
    return bootSetupMode(repo, log, deps, { config, error });
  }
  ...
  const server = createServer({ repo, runtime: { config, board, hiveDir, hooksPath, promptsDir }, state: saved, log, ...deps });
...
async function bootSetupMode(
  repo: string, log: Logger, deps: Pick<ServerDeps, 'readPlanLimits'>, setupFallback?: ServerDeps['setupFallback'],
): Promise<BootedHive> {
  log.info(`boot repo=${repo} mode=setup reason=${setupFallback?.error ?? 'no hive.config.json'}`);
  const server = createServer({ repo, setupFallback, log, ...deps });
